-- Forward fix for persist_extracted_questions.
-- PostgreSQL raises 42702 when the PL/pgSQL variable page_number collides
-- with unnest(failed_pages) AS page_number. This runs on every persist,
-- including empty failed-page arrays, and was the production RPC defect.
-- Do not edit 20260816010000_question_bank_v2.sql.
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1
    FROM unnest(failed_pages) AS failed_page(page_no)
    WHERE failed_page.page_no < 1 OR failed_page.page_no > src.page_count
  ) THEN
  ...
  LOOP
    ...
    END IF;
    position_key := question_page_number::text || ':' || source_order::text;
